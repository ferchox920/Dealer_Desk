...
function printHelp() {
  console.log('Usage: node src/scripts/migrations/identity.cli.js <create|up|down|status> [value]');
}

function formatCliErrorMessage(error) {
  if (!error) {
    return 'Unknown identity migration error.';
  }
  ...
  if (typeof error.code === 'string' && error.code.length > 0) {
    return `Identity migration error (${error.code}).`;
  }
...
async function run() {
  ...
  if (command === 'create') {
    ...
    console.log(`Created identity migration: ${path.relative(process.cwd(), filePath)}`);
    return;
  }
  ...
  if (command === 'up' || command === 'down') {
    ...
    if (migratedFiles.length === 0) {
      console.log('No identity migrations to run.');
      return;
    }

    console.log(`Identity migrations ${command} complete.`);
    return;
  }
...
run().catch((error) => {
  if (error?.code === '3D000') {
    console.error(
      `Identity migration CLI failed: database "${process.env.DB_NAME || 'dealer_desk'}" does not exist yet.`,
    );
    process.exit(1);
  }

  console.error('Identity migration CLI failed:', formatCliErrorMessage(error));
  process.exit(1);
});
