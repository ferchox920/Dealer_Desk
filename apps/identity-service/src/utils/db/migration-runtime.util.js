...
import { getIdentityDatabaseConfig } from './database-config.util.js';

const { Client } = pg;

const TARGET = {
  label: 'Identity DB',
  migrationsDir: path.resolve(process.cwd(), 'src/entities/migrations/identity'),
  migrationsTable: 'identity_schema_migrations',
  requiredCommand: 'npm run migrate:up',
  getDatabaseConfig: getIdentityDatabaseConfig,
};
...
async function ensureIdentityDatabaseReady() {
