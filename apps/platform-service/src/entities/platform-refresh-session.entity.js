...
const PLATFORM_REFRESH_SESSION_SELECT = `
  id,
  platform_admin_id,
  token_hash,
  user_agent,
...
const PlatformRefreshSession = {
  async create(data, executor = query) {
    const { rows } = await executor(
      `
        ...
        VALUES ($1, $2, $3, $4, $5, $6)
        RETURNING ${PLATFORM_REFRESH_SESSION_SELECT}
      `,
      [
        uuidv4(),
        data.platform_admin_id,
  ...
  async findByPlainTokenForUpdate(refreshToken, executor = query) {
    const { rows } = await executor(
      `
        SELECT ${PLATFORM_REFRESH_SESSION_SELECT}
...
export default PlatformRefreshSession;
