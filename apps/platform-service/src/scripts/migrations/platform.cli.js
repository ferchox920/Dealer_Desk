...
function printHelp() {
  console.log('Usage: node src/scripts/migrations/platform.cli.js <create|up|down|status> [value]');
}

function formatCliErrorMessage(error) {
  if (!error) {
    return 'Unknown platform migration error.';
  }
  ...
  if (typeof error.code === 'string' && error.code.length > 0) {
    return `Platform migration error (${error.code}).`;
  }
...
async function run() {
  ...
  if (command === 'create') {
    ...
    console.log(`Created platform migration: ${path.relative(process.cwd(), filePath)}`);
    return;
  }
  ...
  if (command === 'up' || command === 'down') {
    ...
    if (migratedFiles.length === 0) {
      console.log('No platform migrations to run.');
      return;
    }

    console.log(`Platform migrations ${command} complete.`);
    return;
  }
...
run().catch((error) => {
  if (error?.code === '3D000') {
    console.error(
      `Platform migration CLI failed: database "${process.env.PLATFORM_DB_NAME || process.env.DB_NAME || 'dealer_desk'}" does not exist yet.`,
    );
    process.exit(1);
  }

  console.error('Platform migration CLI failed:', formatCliErrorMessage(error));
  process.exit(1);
});
