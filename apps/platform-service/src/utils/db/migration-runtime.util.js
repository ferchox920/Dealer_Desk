...
import { getPlatformDatabaseConfig } from './database-config.util.js';

const { Client } = pg;

const TARGET = {
  label: 'Platform DB',
  migrationsDir: path.resolve(process.cwd(), 'src/entities/migrations/platform'),
  migrationsTable: 'platform_schema_migrations',
  requiredCommand: 'npm run migrate:up',
  getDatabaseConfig: getPlatformDatabaseConfig,
};
...
function splitMigrationSections(content) {
  const downMarkerIndex = content.indexOf(DOWN_MARKER);
  ...
  return {
    ...
    downSql: content.slice(downMarkerIndex + DOWN_MARKER.length).trim(),
  };
}
...
async function ensurePlatformDatabaseReady() {
