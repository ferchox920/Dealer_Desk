...
function mountLegacyAdminRoutesPlaceholder() {
  expressApp.use('/api/admin', (_req, res) => {
    return res.status(410).json({
      status: 410,
      error: 'Legacy monolith admin routes were removed. Use the microservices stack through the gateway instead.',
...
async function startServer() {
  try {
    ...
    mountLegacyAdminRoutesPlaceholder();

    expressApp.listen(PORT, () => {
      console.log(`Server running on port ${PORT}`);
      console.log('Legacy monolith admin routes removed.');
    });
  } catch (error) {
